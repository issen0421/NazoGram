document.addEventListener('DOMContentLoaded', () => {
    ...
    if (projectsContainer && typeof CONFIG !== 'undefined') {
        CONFIG.projects.forEach(p => {
            ...
            if(p.locked) {
                ...
                html = `
                <div class="group bg-nazo-dark border border-gray-800 ${borderHover} transition-all duration-300 overflow-hidden relative rounded cursor-pointer">
    ...
    if (membersContainer && typeof CONFIG !== 'undefined') {
        membersContainer.innerHTML = '';
        CONFIG.members.forEach((m, index) => {
            let colorHex = '';
            let colorClass = '';
            ...
            const html = `
            <div class="group relative bg-nazo-dark border border-gray-800 p-6 rounded-lg hover:translate-y-[-5px] transition-all duration-300 overflow-hidden">
                ...
                <div class="w-16 h-16 bg-gray-800 rounded-full mb-4 flex items-center justify-center border-2 border-nazo-dim group-hover:border-[${colorHex}] transition-colors" style="border-color: #586575;">
                    <span class="font-mono text-xl text-white">0${index + 1}</span>
                </div>
                <h3 class="text-xl font-bold text-white mb-1 font-mono">${m.name}</h3>
                <p class="${colorClass} text-sm mb-4">${m.role}</p>
